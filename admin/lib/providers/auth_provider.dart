...
class AuthUser {
  ...
  const AuthUser({required this.id, this.email, this.role = 'customer'});
  ...
  Future<void> _fetchProfile(String userId) async {
    try {
      ...
      if (result != null) {
        ...
        _user = AuthUser(id: userId, email: email ?? _user?.email, role: role);
      }
    } catch (e) {
      if (kDebugMode) debugPrint('[AuthNotifier] _fetchProfile failed: $e');
    }
  }
  ...
  Future<void> signIn({
    ...
    try {
      ...
      if (result.session != null) {
        _fetchProfile(result.session!.user.id);
      }
  ...
  Future<void> signInWithGoogle() async {
    ...
    try {
      ...
      await supabase.auth.signInWithOAuth(
        ...
        redirectTo: 'homesurge-admin://login',
      ).timeout(const Duration(seconds: 15));
    } catch (e) {
      _user = null;
      state = AuthStatus.unauthenticated;
