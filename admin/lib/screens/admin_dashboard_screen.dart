...
bool shouldShowBathrooms(int? bedrooms, String propertyType) {
...
class AdminDashboardScreen extends ConsumerWidget {
  ...
  Future<void> _exportCsv(WidgetRef ref, List<AdminProperty> properties) async {
    ...
    for (final r in properties) {
      lines.add(headers.map((h) => _csvEscape((r as Map<String, dynamic>)[h])).join(','));
    }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(isAdmin ? 'Admin Dashboard' : 'Publisher Dashboard'),
            ...
            const SizedBox(height: 12),
             ...
             TextField(
               decoration: InputDecoration(
                 hintText: 'Search title, ref, town, phone, address...',
                    ...
                    Row(
                      children: [
                        if (property.bedrooms != null) ...[
                          _MiniChip('${property.bedrooms} bed', Icons.bed_outlined),
                          const SizedBox(width: 4),
                        ],
                        if (shouldShowBathrooms(property.bedrooms, property.propertyType) && property.bathrooms != null) ...[
                          _MiniChip('${property.bathrooms} bath', Icons.bathtub_outlined),
                          const SizedBox(width: 4),
                        ],
                        _MiniChip(property.propertyType, Icons.home_outlined),
                      ],
                    ),
                  ],
                ),
              ...
              Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: property.isPublished ? Colors.green.withValues(alpha: 0.15) : Colors.amber.withValues(alpha: 0.15),
                      borderRadius: BorderRadius.circular(8),
                    ),
                    child: Text(
                      property.isPublished ? 'Live' : 'Draft',
                      style: TextStyle(
                        color: property.isPublished ? Colors.green : Colors.amber,
                        fontSize: 10,
